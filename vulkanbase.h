...
//------<BUFFERS>------
	...
	std::vector<std::array<float, 3>> createNormals(){
	
		//normal vector for every vertex
		std::vector<std::array<float, 3>> normals;
		normals.resize(vertices.size());

		for(int t=0; t+3<indices.size(); t+=3){ //for every triangle in indices
			auto a = vertices[indices[t]].pos; //retrieve vertex coordinates
			auto b = vertices[indices[t+1]].pos;
			auto c = vertices[indices[t+2]].pos;

			std::array<float, 3> n; //normal
			n[0] = (a[1]-b[1])*(a[2]-c[2]) - (a[2]-b[2])*(a[1]-c[1]);
			n[1] = (a[2]-b[2])*(a[0]-c[0]) - (a[0]-b[0])*(a[2]-c[2]);
			...
			printf("triangle normal: {%.3f, %.3f, %.3f}},\n", n[0], n[1], n[2]);
			for(int i=0; i<3; i++){ //add normal to vertices from triangle
				normals[indices[t+i]][0] += abs(n[0]);
				normals[indices[t+i]][1] += abs(n[1]);
				normals[indices[t+i]][2] += abs(n[2]);
/*
*/
			}
		}
